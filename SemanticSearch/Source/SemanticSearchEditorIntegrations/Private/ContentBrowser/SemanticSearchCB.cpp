...
namespace Private
{
	...
	public:
		...
		virtual void OnItemsAvailable(TArrayView<const TSharedPtr<FAssetViewItem>> Items) override
		{
			...
			if (bHasActiveResults && bHasInFlightRequest)
			{
				...
				UE_LOG(LogSemanticSearchCB, Verbose, TEXT("OnItemsAvailable: finalizing in-flight request (Epoch=%u, Query='%ls', IncomingItems=%d, CurrentItems=%d, LastMatched=%d)"),
			...
			{
				UE_LOG(LogSemanticSearchCB, Verbose, TEXT("OnItemsAvailable: empty query passing items through unfiltered (Epoch=%u, IncomingItems=%d, CurrentItems=%d)"),
			...
			{
				UE_LOG(LogSemanticSearchCB, Verbose, TEXT("OnItemsAvailable: re-publishing matching items for late-arriving batch (Epoch=%u, Query='%ls', IncomingItems=%d, LastMatched=%d)"),
		...
		{
			UE_LOG(LogSemanticSearchCB, Verbose, TEXT("OnAllKnownItemsAvailable (Epoch=%u, Query='%ls', CurrentItems=%d, PendingEmbedding=%ls)"),
	...
	protected:
		...
		{
			UE_LOG(LogSemanticSearchCB, Verbose, TEXT("Filtering reset — invalidating in-flight async work (OldEpoch=%u, NewEpoch=%u, Query='%ls', CurrentItems=%d, LastMatched=%d, HadInFlight=%ls, HadActiveResults=%ls)"),
	...
	private:
		...
		void HandleFindSimilar(const FString& AssetPath)
		{
			...
			{
				UE_LOG(LogSemanticSearchCB, Warning, TEXT("Find Similar: index is empty, nothing to compare against (%ls)"), *AssetPath);
			...
			FHybridSearchIndex::Get().ExtractEmbeddingsAsync({ AssetID },
				[WeakSelf, Epoch, AssetPath](TArray<int64>&& IDs, TArray<float>&& Vectors, int32 Dimension) mutable
				{
					...
					{
						UE_LOG(LogSemanticSearchCB, Warning, TEXT("Find Similar: '%ls' is not in the semantic index — returning no results"), *AssetPath);
						Self->FinalizeEmptyResults();
						return;
					}


					UE_LOG(LogSemanticSearchCB, Verbose, TEXT("Find Similar: extracted embedding for '%ls' (Epoch=%u, Dim=%d, VectorCount=%d)"),
		...
		void SubmitQueryAsync(FString QueryText)
		{
			...
			if (!Provider)
			{
				UE_LOG(LogSemanticSearchCB, Warning, TEXT("No embedding provider registered."));
				return;
			}
			...
			Provider->GenerateEmbeddingAsync(QueryText, [WeakSelf, Epoch](FEmbeddingResponse&& Response) mutable
			{
				...
				AsyncTask(ENamedThreads::GameThread, [WeakSelf, Epoch, Response = MoveTemp(Response)]() mutable
				{
					...
					{
						UE_LOG(LogSemanticSearchCB, Verbose, TEXT("Query embedding generated (Epoch=%u, Query='%ls', EmbeddingSize=%d)"),
							Epoch, *Self->ActiveQuery.ToString(), Response.Embedding.Num());
						Self->DispatchHybridSearch(MoveTemp(Response.Embedding), Epoch);
					}
					else
					{
						UE_LOG(LogSemanticSearchCB, Warning, TEXT("Query embedding failed: %ls"), *Response.ErrorMessage);
		...
		void DispatchHybridSearch(TArray<float>&& QueryEmbedding, uint32 Epoch)
		{
			...
			UE_LOG(LogSemanticSearchCB, Verbose, TEXT("DispatchHybridSearch: running hybrid search (Epoch=%u, Query='%ls', CurrentItems=%d, Similarity=%ls)"),
				 ...
				 ItemPtrs = MoveTemp(ItemPtrs)]() mutable
				{
					...
					ISemanticSearchModule::Get().SearchAsync(
						 ...
						 EntryIDs = MoveTemp(EntryIDs), EntryItems = MoveTemp(EntryItems), IDHashTable = MoveTemp(IDHashTable)](TArray<FHybridSearchResult>&& Results) mutable
						{
							...
							UE_LOG(LogSemanticSearchCB, Verbose, TEXT("SearchAsync round-trip latency %.4f ms (Epoch=%u, Results=%d)"),
								 ...
								 Results = MoveTemp(Results)]() mutable
								{
									...
									AsyncTask(ENamedThreads::GameThread,
										 ...
										 ScoreHashTable = MoveTemp(ScoreHashTable)]() mutable
										{
											...
											{
												UE_LOG(LogSemanticSearchCB, Verbose, TEXT("Publish: publishing %d matching items (Epoch=%u, Query='%ls', Similarity=%ls)"),
											...
											UE_LOG(LogSemanticSearchCB, Verbose, TEXT("DispatchHybridSearch: end-to-end latency %.4f ms (Epoch=%u, Query='%ls', Published=%d)"),
