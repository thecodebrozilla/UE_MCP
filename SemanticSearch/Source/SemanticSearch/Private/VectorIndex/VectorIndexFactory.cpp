...
TSharedPtr<IVectorIndex> DeserializeVectorIndex(ESemanticSearchIndexType Type, TConstArrayView<uint8> Data, int32 Dimension)
{
	...
	default:
		UE_LOG(LogSemanticSearch, Warning, TEXT("DeserializeVectorIndex: unsupported index type %d"), static_cast<uint8>(Type));
		return nullptr;
	}
}
...
TSharedPtr<IVectorIndex> DeserializeCodebook(ESemanticSearchIndexType Type, TConstArrayView<uint8> Data, int32 Dimension)
{
	...
	default:
		UE_LOG(LogSemanticSearch, Warning, TEXT("DeserializeCodebook: unsupported index type %d"), static_cast<uint8>(Type));
		return nullptr;
	}
}
