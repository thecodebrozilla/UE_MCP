...
void USemanticSearchSettings::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
		...
		|| PropertyName == GET_MEMBER_NAME_CHECKED(USemanticSearchSettings, EmbeddingDimension))
	{
		...
		UE::SemanticSearch::ISemanticSearchModule::Get().RegisterEmbeddingProvider(CreateConfiguredEmbeddingProvider(), true);
		UE_LOG(LogSemanticSearch, Log, TEXT("Re-registered embedding provider after settings change (%ls)."),
...
{
	UE_LOG(LogSemanticSearch, Verbose, TEXT("SemanticSearch module starting up"));
	...
	{
		UE_LOG(LogSemanticSearch, Log, TEXT("SemanticSearch.DelayInitialization=1; waiting for explicit Initialize() call"));
	}
}
...
void FSemanticSearchModule::Initialize()
{
	...
	if (bIsInitialized)
	{
		UE_LOG(LogSemanticSearch, Verbose, TEXT("Initialize: already initialized; ignoring"));
		return;
	}
	...
	if (Index.LoadFromFile(IndexPath, LoadedPackageHashes))
	{
		...
		UE_LOG(LogSemanticSearch, Log, TEXT("Loaded saved index from %ls (vector: %d, BM25: %d, hashes: %d)"),
		...
		{
			UE_LOG(LogSemanticSearch, Log, TEXT("Saved index dimension %d differs from registered provider dimension %d; rebuilding as Flat"),
...
{
	UE_LOG(LogSemanticSearch, Verbose, TEXT("SemanticSearch module shutting down"));
		...
		bStarted = FAssetProcessorManager::Get().GetQuantizedIndexingData(AssetData, CodebookHash,
			[Asset = FAssetData(AssetData), CurIndexId, CapturedEpoch](FAssetIndexingResult&& Result, FString&& Error, EAssetIndexFailureReason Reason) mutable
			{
				if (!Error.IsEmpty() || Result.QuantizedCodes.IsEmpty())
				{
					...
					{
						UE_LOG(LogSemanticSearch, Verbose, TEXT("Failed to get quantized codes for %ls: %ls"),
			...
			[Asset = FAssetData(AssetData), bBuildOnMiss, CurIndexId, CapturedEpoch](FAssetIndexingResult&& Result, FString&& Error, EAssetIndexFailureReason Reason) mutable
			{
				if (!Error.IsEmpty() || Result.Embedding.IsEmpty())
				{
					...
					if (Reason != EAssetIndexFailureReason::None)
					{
						if (bBuildOnMiss)
						{
							UE_LOG(LogSemanticSearch, Verbose, TEXT("Indexing failed for %ls: %ls"),
...
void FSemanticSearchModule::OnAssetAdded(const FAssetData& AssetData)
{
	...
	FHybridSearchIndex::Get().IsFailedAsync({ID},
		[this, AssetData = FAssetData(AssetData), ID](TSet<int64>&& FailedIDs)
		{
			...
			{
				const USemanticSearchSettings* Settings = USemanticSearchSettings::Get();
				UE_LOG(LogSemanticSearch, Verbose, TEXT("OnAssetAdded: %ls"), *AssetData.GetObjectPathString());
...
void FSemanticSearchModule::OnPackageSaved(const FString& PackageFileName, UPackage* Package, FObjectPostSaveContext ObjectSaveContext)
{
	...
	UE_LOG(LogSemanticSearch, Verbose, TEXT("OnPackageSaved: %ls (deferring until AR matches hash)"), *PackageName.ToString());
...
void FSemanticSearchModule::OnPackageSavedDeferred(FName PackageName, FIoHash ExpectedHash, int32 RetryCount)
{
	...
	if (Result != UE::AssetRegistry::EExists::Exists || AssetPackageData.GetPackageSavedHash() != ExpectedHash)
	{
		...
		{
			UE_LOG(LogSemanticSearch, Verbose, TEXT("OnPackageSavedDeferred: AR not up to date for %ls yet, retrying (%d/%d)"),
		...
		{
			UE_LOG(LogSemanticSearch, Verbose, TEXT("OnPackageSavedDeferred: AR hash never matched for %ls after %d retries, giving up"),
	...
	for (const FAssetData& AssetData : Assets)
	{
		...
		{
			UE_LOG(LogSemanticSearch, Verbose, TEXT("OnPackageSavedDeferred: indexing %ls"), *AssetData.GetObjectPathString());
...
void FSemanticSearchModule::OnAssetRenamed(const FAssetData& NewAssetData, const FString& OldObjectPath)
{
	...
	UE_LOG(LogSemanticSearch, Verbose, TEXT("OnAssetRenamed: %ls -> %ls (OldInFolder=%d, NewInFolder=%d)"),
	...
	if (bOldInFolder && bNewInFolder)
	{
		...
		UE_LOG(LogSemanticSearch, Verbose, TEXT("OnAssetRenamed: enqueuing update for %ls -> %ls"),
	...
	{
		UE_LOG(LogSemanticSearch, Verbose, TEXT("OnAssetRenamed: removed %ls from index (moved out of indexed folder)"),
			*OldObjectPath);
		FHybridSearchIndex::Get().RemoveByPath(FString(OldObjectPath));
	}
	else
	{
		UE_LOG(LogSemanticSearch, Verbose, TEXT("OnAssetRenamed: moved into indexed folder, indexing %ls"),
...
void FSemanticSearchModule::RegisterEmbeddingProvider(TUniquePtr<IEmbeddingProvider>&& EmbeddingProvider, bool bRebuildIndex)
{
	...
	{
		UE_LOG(LogSemanticSearch, Warning, TEXT("RegisterEmbeddingProvider: ignoring null provider"));
		return;
	}
	...
	if (NewDimension > 0)
	{
		...
		if (bIsInitialized && bRebuildIndex)
		{
			...
			UE_LOG(LogSemanticSearch, Log, TEXT("Embedding provider replaced (NewDim=%d, OldDim=%d): rebuilt vector index as Flat"),
	...
	{
		UE_LOG(LogSemanticSearch, Warning, TEXT("RegisterEmbeddingProvider: new provider reports dimension %d; leaving hybrid index untouched and keep the previous provider"),
			NewDimension);
	}
}
...
void FSemanticSearchModule::OnFilesLoaded()
{
	...
	// 	}
	// 	UE_LOG(LogSemanticSearch, Log, TEXT("Removed %d orphaned entries from loaded index"), LoadedPackageHashes.Num());
	// 	LoadedPackageHashes.Empty();
	// }
	...
	{
		UE_LOG(LogSemanticSearch, Log, TEXT("Auto-indexing on startup (bBuildUncached=%d)"), Settings->bAutoIndexUncachedOnStartup);
...
void FSemanticSearchModule::SwitchIndexType(ESemanticSearchIndexType NewType)
{
	...
	{
		UE_LOG(LogSemanticSearch, Warning, TEXT("SwitchIndexType: Already switching, ignoring request"));
		return;
	}
...
void FSemanticSearchModule::SwitchToFlatIndex()
{
	...
	UE_LOG(LogSemanticSearch, Log, TEXT("Switched to Flat index, re-indexing all assets from DDC..."));
...
void FSemanticSearchModule::SwitchToQuantizedIndex(ESemanticSearchIndexType NewType, bool bForceTrain)
{
	...
	if (Dimension == 0)
	{
		UE_LOG(LogSemanticSearch, Warning, TEXT("SwitchToQuantizedIndex: Current index dimension zero, cannot switch"));
		bIsSwitchingIndexType = false;
	...
	if (!bForceTrain)
	{
		...
		if (FFileHelper::LoadFileToArray(CodebookData, *CodebookPath))
		{
			...
			if (NewIndex)
			{
				UE_LOG(LogSemanticSearch, Log, TEXT("Loaded existing codebook from %ls, skipping training"), *CodebookPath);
			}
		}
	}
	...
	if (NewIndex)
	{
		...
		UE_LOG(LogSemanticSearch, Log, TEXT("Switched to quantized index with existing codebook, re-indexing from DDC..."));
		bIsSwitchingIndexType = false;
	...
	else
	{
		...
		{
			UE_LOG(LogSemanticSearch, Warning, TEXT("SwitchToQuantizedIndex: No vectors available for training"));
			bIsSwitchingIndexType = false;
			FHybridSearchIndex::Get().OnIndexChanged().Broadcast(false);
			return;
		}
		...
		Index.ExtractEmbeddingsAsync(MoveTemp(CandidateIDs),
			[this, NewType, Dimension, Settings](TArray<int64>&& AllIDs, TArray<float>&& TrainingVectors, int32 Dim)
			{
				if (AllIDs.Num() == 0)
				{
					UE_LOG(LogSemanticSearch, Warning, TEXT("SwitchToQuantizedIndex: Could not retrieve any vectors for training"));
					bIsSwitchingIndexType = false;
					FHybridSearchIndex::Get().OnIndexChanged().Broadcast(false);
					return;
				}
				...
				UE_LOG(LogSemanticSearch, Log, TEXT("Training quantized index (type=%d) with %d vectors (dim=%d)..."),
				...
				Async(EAsyncExecution::ThreadPool,
					[this, NewType, TrainedIndex, TrainingVectors = MoveTemp(TrainingVectors), NumVectors = AllIDs.Num(), SwitchProgress]() mutable
					{
						...
						if (!CodebookPath.IsEmpty())
						{
							...
							{
								UE_LOG(LogSemanticSearch, Log, TEXT("Saved codebook to %ls"), *CodebookPath);
							}
							else
							{
								UE_LOG(LogSemanticSearch, Warning, TEXT("Failed to save codebook to %ls"), *CodebookPath);
							}
						}
						...
						UE_LOG(LogSemanticSearch, Log, TEXT("Quantized index trained, re-indexing %d assets from DDC..."), NumVectors);
...
void FSemanticSearchModule::RetrainIndex()
{
	...
	{
		UE_LOG(LogSemanticSearch, Warning, TEXT("RetrainIndex: Current index is Flat, nothing to retrain"));
		return;
	}

	if (bIsSwitchingIndexType)
	{
		UE_LOG(LogSemanticSearch, Warning, TEXT("RetrainIndex: Already switching/retraining, ignoring request"));
		return;
	}
