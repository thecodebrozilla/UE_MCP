	...
	: HttpClient(FSemanticHttpClient::CreateFromSettings())
{
	...
	UE_LOG(LogSemanticSearch, Log, TEXT("OpenAIEmbeddingProvider created (CaptionModel: %ls @ %ls, EmbedModel: %ls @ %ls, Dim: %d%ls%ls)"),
	...
	DerivedData::FRequestOwner* RequestOwner)
{
	...
	for (const FAssetMedia& Media : Request.AssetMedia)
	{
		...
		{
			UE_LOG(LogSemanticSearch, Verbose, TEXT("Skipping unsupported media type: %ls"), *Media.MimeType);
		}
	}
