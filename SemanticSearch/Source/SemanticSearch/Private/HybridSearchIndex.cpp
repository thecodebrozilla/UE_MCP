...
void FHybridSearchIndex::EnsureInitialized(int32 EmbeddingDimension)
{
	...
	if (!CodebookPath.IsEmpty())
	{
			...
			FFileHelper::LoadFileToArray(CodebookData, *CodebookPath))
		{
			...
			if (VectorIndex)
			{
				UE_LOG(LogSemanticSearch, Log, TEXT("Loaded codebook from %ls"), *CodebookPath);
	...
	UE_LOG(LogSemanticSearch, Display, TEXT("No codebook found, creating flat index"));
...
void FHybridSearchIndex::WriteBytesToFile(const FString& FilePath, TArray64<uint8> Bytes)
{
	...
	if (!Writer)
	{
		UE_LOG(LogSemanticSearch, Warning, TEXT("Failed to open semantic-search file for write: %ls"), *FilePath);
		return;
	}
	...
	if (!Writer->Close())
	{
		UE_LOG(LogSemanticSearch, Warning, TEXT("Failed to write semantic-search file: %ls"), *FilePath);
	}
}
...
void FHybridSearchIndex::LoadPreProcessorFailedFromFile(const FString& FilePath)
{
	...
	{
		UE_LOG(LogSemanticSearch, Warning, TEXT("Failed to read pre-processor failed sidecar file: %ls"), *FilePath);
		PreProcessorFailedAssets.Empty();
		return;
	}
	...
	if (Ar.IsError())
	{
		UE_LOG(LogSemanticSearch, Warning, TEXT("Pre-processor failed sidecar file is corrupt, ignoring: %ls"), *FilePath);
		PreProcessorFailedAssets.Empty();
		return;
	}
...
bool FHybridSearchIndex::LoadFromFile(const FString& FilePath, TMap<int64, FIoHash>& OutPackageHashes)
{
	...
	if (bLoadedInitialized && VectorDim > 0 && VectorData.Num() > 0)
	{
		...
		{
			UE_LOG(LogSemanticSearch, Warning, TEXT("Saved index type (%d) does not match current settings (%d), discarding"),
		...
		if (SavedType != ESemanticSearchIndexType::Flat && SavedCodebookHash != FIoHash())
		{
			...
			if (FFileHelper::LoadFileToArray(CodebookData, *CodebookPath))
			{
				...
				if (TempIndex && TempIndex->SupportsQuantization())
				{
					...
					{
						UE_LOG(LogSemanticSearch, Warning, TEXT("Saved index codebook hash does not match current codebook, discarding saved index"));
						return false;
					}
				}
			}
