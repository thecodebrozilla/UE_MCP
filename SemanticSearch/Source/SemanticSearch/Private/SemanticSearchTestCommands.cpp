...
static TArray<FAssetData> GetAssetsInIndexedFolders(IAssetRegistry& AssetRegistry)
{
	...
	{
		UE_LOG(LogSemanticSearch, Warning, TEXT("GetAssetsInIndexedFolders: no indexed paths configured in Semantic Search settings"));
		return {};
	}
	...
	for (const FDirectoryPath& Dir : Settings->IndexedPaths)
	{
		...
		UE_LOG(LogSemanticSearch, Log, TEXT("GetAssetsInIndexedFolders: '%ls' -> %d assets (%d new)"), *Dir.Path, FolderAssets.Num(), Added);
	}
	return Result;
}
...
static FAutoConsoleCommand IndexStatsCommand(
	...
	FConsoleCommandDelegate::CreateLambda([]()
	{
		...
		{
			UE_LOG(LogSemanticSearch, Warning, TEXT("No vector index is registered."));
			return;
		}
		...
		Searcher.ContainsAsync(MoveTemp(IDs),
			[Total, Stats](TSet<int64>&& ContainedIDs)
			{
				...
				UE_LOG(LogSemanticSearch, Log, TEXT("Index: %d entries | Folder assets: %d total, %d indexed, %d not indexed (%d dims)"),
...
static FAutoConsoleCommand IndexAllCommand(
	...
	FConsoleCommandDelegate::CreateLambda([]()
	{
		...
		IAssetRegistry& AssetRegistry = FModuleManager::LoadModuleChecked<FAssetRegistryModule>(TEXT("AssetRegistry")).Get();
		TArray<FAssetData> Assets = GetAssetsInIndexedFolders(AssetRegistry);
		UE_LOG(LogSemanticSearch, Log, TEXT("IndexAll: found %d assets in indexed folders"), Assets.Num());
		...
		Searcher.ContainsAsync(MoveTemp(IDs),
			[Assets = MoveTemp(Assets)](TSet<int64>&& ContainedIDs)
			{
				...
				UE_LOG(LogSemanticSearch, Log, TEXT("IndexAll: queuing %d assets for indexing, %d already indexed"),
...
static FAutoConsoleCommand TestSearchCommand(
	...
	FConsoleCommandWithArgsDelegate::CreateLambda([](const TArray<FString>& Args)
	{
		if (Args.Num() < 1)
		{
			UE_LOG(LogSemanticSearch, Warning, TEXT("Usage: SemanticSearch.TestSearch <query text> [K=10]"));
			return;
		}
		...
		UE_LOG(LogSemanticSearch, Log, TEXT("Searching for: \"%ls\" (K=%d)"), *QueryText, K);
		...
		if (!Provider.IsValid())
		{
			UE_LOG(LogSemanticSearch, Warning, TEXT("No embedding provider registered."));
			return;
		}
		Provider->GenerateEmbeddingAsync(QueryText,
			[K, QueryText, IDToPath = MoveTemp(IDToPath)](const FEmbeddingResponse& Response)
			{
				...
				{
					UE_LOG(LogSemanticSearch, Error, TEXT("Embedding failed: %ls"), *Response.ErrorMessage);
					return;
				}
				...
				FHybridSearchIndex::Get().SearchAsync(
					...
					{
						UE_LOG(LogSemanticSearch, Log, TEXT("Top %d results:"), Results.Num());
						for (int32 i = 0; i < Results.Num(); ++i)
						{
							const FString* Path = IDToPath.Find(Results[i].ID);
							UE_LOG(LogSemanticSearch, Log, TEXT("  [%d] rrf=%.4f  vec_dist=%.4f  bm25=%.4f  %ls"),
								i + 1,
								Results[i].RRFScore,
...
static FAutoConsoleCommand TestCaptionCommand(
	...
	FConsoleCommandWithArgsDelegate::CreateLambda([](const TArray<FString>& Args)
	{
		if (Args.Num() < 1)
		{
			UE_LOG(LogSemanticSearch, Warning, TEXT("Usage: SemanticSearch.TestCaption <image_file_path>"));
			return;
		}
		...
		{
			UE_LOG(LogSemanticSearch, Error, TEXT("File not found: %ls"), *FilePath);
			return;
		}
		...
		{
			UE_LOG(LogSemanticSearch, Error, TEXT("Failed to load file: %ls"), *FilePath);
			return;
		}
		...
		UE_LOG(LogSemanticSearch, Log, TEXT("Sending caption request (%ls, %d bytes)..."), *MimeType, Request.AssetMedia[0].Data.Num());
		...
		Provider->GenerateCaptionAsync(MoveTemp(Request),
			[](const FCaptionResponse& Response)
			{
				if (!Response.ErrorMessage.IsEmpty())
				{
					UE_LOG(LogSemanticSearch, Error, TEXT("Caption failed: %ls"), *Response.ErrorMessage);
					return;
				}

				UE_LOG(LogSemanticSearch, Log, TEXT("Caption: %ls"), *Response.Caption);
				if (Response.Keywords.Num() > 0)
				{
					UE_LOG(LogSemanticSearch, Log, TEXT("Keywords: %ls"), *FString::Join(Response.Keywords, TEXT(", ")));
				}
			}, nullptr);
	})
);
...
static FAutoConsoleCommand TestEmbeddingCommand(
	...
	FConsoleCommandWithArgsDelegate::CreateLambda([](const TArray<FString>& Args)
	{
		...
		UE_LOG(LogSemanticSearch, Log, TEXT("Generating embedding for: \"%ls\""), *Text);
		...
		Provider->GenerateEmbeddingAsync(Text,
			[](const FEmbeddingResponse& Response)
			{
				...
				UE_LOG(LogSemanticSearch, Log, TEXT("Embedding: %d dimensions"), Response.Embedding.Num());
				if (!Response.ModelVersion.IsEmpty())
				{
					UE_LOG(LogSemanticSearch, Log, TEXT("Model: %ls"), *Response.ModelVersion);
				}
				...
				if (Response.Embedding.Num() > 0)
				{
					...
					UE_LOG(LogSemanticSearch, Log, TEXT("First %d values: [%ls, ...]"), Preview, *Vals);
				}
			}, nullptr);
	})
);
...
static FAutoConsoleCommand ShowAssetCaptionCommand(
	...
	FConsoleCommandWithArgsDelegate::CreateLambda([](const TArray<FString>& Args)
	{
		if (Args.Num() != 1)
		{
			UE_LOG(LogSemanticSearch, Warning, TEXT("Usage: SemanticSearch.ShowAssetCaption <asset object path>"));
			return;
		}
		...
		{
			UE_LOG(LogSemanticSearch, Warning, TEXT("Asset not found: %ls"), *Args[0]);
			return;
		}
		...
		const bool bDispatched = FAssetProcessorManager::Get().GetIndexingData(AssetData,
			[AssetPath = Args[0]](FAssetIndexingResult&& Result, FString&& Error, EAssetIndexFailureReason Reason) mutable
			{
				if (!Error.IsEmpty())
				{
					UE_LOG(LogSemanticSearch, Warning, TEXT("No indexed caption for %ls (%ls)"), *AssetPath, *Error);
					return;
				}
				UE_LOG(LogSemanticSearch, Log, TEXT("Caption: %ls"), Result.Caption.IsEmpty() ? TEXT("<empty>") : *Result.Caption);
				if (Result.Keywords.Num() > 0)
				{
					UE_LOG(LogSemanticSearch, Log, TEXT("Keywords: %ls"), *FString::Join(Result.Keywords, TEXT(", ")));
				}
			}, /*bBuildOnMiss=*/false);

		if (!bDispatched)
		{
			UE_LOG(LogSemanticSearch, Warning, TEXT("No processor for asset type '%ls'"), *AssetData.AssetClassPath.ToString());
		}
	})
);
