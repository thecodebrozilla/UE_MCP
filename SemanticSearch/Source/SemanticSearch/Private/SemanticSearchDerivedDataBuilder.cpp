...
#else
	// [5.7 port] ECachePolicy::NonDeterministic is new in UE 5.8. Default omits it in 5.7.
	return ECachePolicy::Default;
#endif
}
								...
								EmbeddingProvider->GenerateEmbeddingAsync(CaptionString, [this, UsedCaption = MoveTemp(Caption), SelfKeepAlive = AsShared()](FEmbeddingResponse&& Embedding) mutable
									{
										...
										if (Embedding.ErrorMessage.IsEmpty())
										{
												...
												DerivedData::GetCache().Put(
													...
													*this,
													[AssetPtr = Asset](DerivedData::FCachePutResponse&& Response)
													{
														...
														if (Response.Status == EStatus::Ok)
														{
															UE_LOG(LogSemanticSearch, Verbose, TEXT("Successfully stored semantic data in DDC for: %ls"), *AssetPtr->GetObjectPathString());
														}
														else
														{
															UE_LOG(LogSemanticSearch, Verbose, TEXT("Failed to store data in DDC for: %ls (%ls)"), *AssetPtr->GetObjectPathString(), *WriteToString<256>(Response.Status));
														}
													});
											...
											{
												UE_LOG(LogSemanticSearch, Verbose, TEXT("Data used to generate the semantic data changed while building it for: %ls, Result won't be saved to DDC since they might be inaccurate"), *Asset->GetObjectPathString());
												BroadcastQueuedRequests(MoveTemp(EmbeddingDerivedData), MoveTemp(CaptionDerivedData), FString(), EAssetIndexFailureReason::None);
											}
										}
										else
				...
				[this, CodebookHash, SelfKeepAlive = AsShared()](TArray<uint8>&& QuantizedCodes, bool bSuccess)
				{
					...
					GetCache().Put({ PutRequest }, *this,
						[AssetPtr = Asset](FCachePutResponse&& Response)
						{
							...
							{
								UE_LOG(LogSemanticSearch, Verbose, TEXT("Failed to store quantized codes in DDC for: %ls"), *AssetPtr->GetObjectPathString());
							}
						});
