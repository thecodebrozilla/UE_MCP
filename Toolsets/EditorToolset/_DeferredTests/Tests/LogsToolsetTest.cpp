...
void FLogsToolsetSpec::Define()
{
	...
	Describe(TEXT("GetLogCategories"), [this]()
	{
		...
		{
			UE_LOG(LogLogsToolsetTest, Log, TEXT("GetLogCategories probe"));
			GLog->Flush();
		...
		{
			UE_LOG(LogLogsToolsetTest, Log, TEXT("Category presence probe"));
			GLog->Flush();
		...
		{
			UE_LOG(LogLogsToolsetTest, Log, TEXT("Filter probe"));
			GLog->Flush();
	...
	Describe(TEXT("GetLogEntries"), [this]()
	{
		...
		{
			UE_LOG(LogLogsToolsetTest, Log, TEXT("Category filter probe"));
			GLog->Flush();
		...
		It(TEXT("Filters by pattern"), [this]()
		{
			...
			UE_LOG(LogLogsToolsetTest, Log, TEXT("%ls"), *Marker);
			GLog->Flush();
		...
		It(TEXT("Respects MaxEntries limit"), [this]()
		{
			...
			{
				UE_LOG(TestLogCategory, Verbose, TEXT("Log testing %d"), i);
			}
	...
	Describe(TEXT("GetVerbosity"), [this]()
	{
		...
		{
			UE_LOG(LogLogsToolsetTest, Log, TEXT("GetVerbosity probe"));
