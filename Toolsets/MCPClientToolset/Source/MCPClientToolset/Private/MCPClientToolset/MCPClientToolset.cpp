...
void FMCPClientToolset::StartSSEConnection()
{
	...
	UE_LOG(LogMCPClientToolset, Log, TEXT("MCPClientToolset '%ls': connecting to %ls"), *Config.Name, *SSEUrl);
	...
	if (!bStreamSet)
	{
		UE_LOG(LogMCPClientToolset, Warning, TEXT("MCPClientToolset '%ls': SetResponseBodyReceiveStreamDelegateV2 failed — SSE streaming unavailable"),
			*Config.Name);
	}
	...
	FHttpRequestPtr /*Request*/, FHttpResponsePtr Response, bool bConnectedSuccessfully)
{
	...
	UE_LOG(LogMCPClientToolset, Log, TEXT("MCPClientToolset '%ls': SSE request ended — connected=%d HTTP=%d"),
...
void FMCPClientToolset::DispatchSSEEvent(const FString& EventType, const FString& Data)
{
	if (EventType == TEXT("endpoint"))
	{
		...
		UE_LOG(LogMCPClientToolset, Log, TEXT("MCPClientToolset '%ls': message endpoint = %ls"), *Config.Name, *MessageEndpointUrl);
			...
			.Next([WeakThis = AsWeak()](TSharedPtr<FJsonObject> InitResponse)
			{
				...
				StrongThis->SendRequest(TEXT("tools/list"), nullptr)
					.Next([WeakThis](TSharedPtr<FJsonObject> ToolsResponse)
					{
						...
						UE_LOG(LogMCPClientToolset, Log, TEXT("MCPClientToolset '%ls': registered %d tool(s)"),
	...
	else if (EventType == TEXT("message"))
	{
		...
		{
			UE_LOG(LogMCPClientToolset, Warning, TEXT("MCPClientToolset '%ls': failed to parse SSE message JSON"), *Config.Name);
			return;
		}
	...
	const FString& Method, TSharedPtr<FJsonObject> Params)
{
	...
	{
		UE_LOG(LogMCPClientToolset, Warning, TEXT("MCPClientToolset '%ls': SendRequest called before endpoint is known (method=%ls)"),
			*Config.Name, *Method);
...
{
	UE_LOG(LogMCPClientToolset, Log, TEXT("MCPClientToolset '%ls': starting Streamable HTTP handshake at %ls"),
	...
	const FString& Method, TSharedPtr<FJsonObject> Params)
{
	...
	UE_LOG(LogMCPClientToolset, Log, TEXT("MCPClientToolset '%ls': SendStreamableHTTPRequest URL='%ls' len=%d method=%ls"),
...
void FMCPClientToolset::StartOAuthFlow()
{
	...
	UE_LOG(LogMCPClientToolset, Log, TEXT("MCPClientToolset '%ls': starting OAuth 2.0 discovery at %ls"),
	...
	FHttpRequestPtr /*Req*/, FHttpResponsePtr Response, bool bSuccess)
{
	...
	if (!AccessToken.IsEmpty())
	{
		UE_LOG(LogMCPClientToolset, Log, TEXT("MCPClientToolset '%ls': using cached OAuth access token"), *Config.Name);
...
void FMCPClientToolset::RegisterOAuthClient()
{
	...
	UE_LOG(LogMCPClientToolset, Log, TEXT("MCPClientToolset '%ls': registering OAuth client at %ls"),
	...
	FHttpRequestPtr /*Req*/, FHttpResponsePtr Response, bool bSuccess)
{
	...
	UE_LOG(LogMCPClientToolset, Log, TEXT("MCPClientToolset '%ls': dynamic client registration succeeded (client_id=%ls)"),
...
void FMCPClientToolset::LaunchOAuthBrowser()
{
	...
	UE_LOG(LogMCPClientToolset, Log, TEXT("MCPClientToolset '%ls': browser opened — waiting for user authorization at http://localhost:%d/callback"),
	...
	FHttpRequestPtr /*Req*/, FHttpResponsePtr Response, bool bSuccess)
{
	...
	UE_LOG(LogMCPClientToolset, Log, TEXT("MCPClientToolset '%ls': OAuth token obtained"), *Config.Name);
...
{
	UE_LOG(LogMCPClientToolset, Log, TEXT("MCPClientToolset '%ls': attempting silent OAuth token refresh"), *Config.Name);
	...
	FHttpRequestPtr /*Req*/, FHttpResponsePtr Response, bool bSuccess)
{
	...
	if (!bSuccess || !Response.IsValid() || !EHttpResponseCodes::IsOk(Response->GetResponseCode()))
	{
		UE_LOG(LogMCPClientToolset, Log, TEXT("MCPClientToolset '%ls': token refresh failed (HTTP %d), falling back to browser login"),
	...
	if (!FJsonSerializer::Deserialize(Reader, Json) || !Json.IsValid())
	{
		UE_LOG(LogMCPClientToolset, Log, TEXT("MCPClientToolset '%ls': token refresh response parse failed, falling back to browser login"),
			*Config.Name);
		FallBackToBrowser();
		return;
	}
	...
	if (!Json->TryGetStringField(TEXT("access_token"), NewAccessToken) || NewAccessToken.IsEmpty())
	{
		UE_LOG(LogMCPClientToolset, Log, TEXT("MCPClientToolset '%ls': token refresh response missing access_token, falling back to browser login"),
			*Config.Name);
		FallBackToBrowser();
		return;
	}
	...
	UE_LOG(LogMCPClientToolset, Log, TEXT("MCPClientToolset '%ls': OAuth token refreshed silently"), *Config.Name);
...
{
	UE_LOG(LogMCPClientToolset, Warning, TEXT("MCPClientToolset: OAuth failed — %ls"), *Reason);
