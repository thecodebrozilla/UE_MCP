	...
	for (const FMCPServerConfig& ServerConfig : Settings->MCPServers)
	{
		...
		UE::ToolsetRegistry::FMCPClientToolset::Create(MCPConfig)
			.Next([WeakThis](TValueOrError<TSharedPtr<UE::ToolsetRegistry::FMCPClientToolset>, FString> Result)
			{
				...
				if (Result.HasError())
				{
					UE_LOG(LogMCPClientToolset, Warning, TEXT("MCPClientToolset: failed to connect — %ls"), *Result.GetError());
					return;
				}
				...
				{
					UE_LOG(LogMCPClientToolset, Warning, TEXT("MCPClientToolset: could not register '%ls' — %ls"),
				...
				UE_LOG(LogMCPClientToolset, Log, TEXT("MCPClientToolset: registered '%ls'"), *Toolset->GetToolsetName());
			});
	}
}
