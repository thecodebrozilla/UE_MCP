...
#if UE_VERSION_OLDER_THAN(5, 8, 0)
	// [5.7 port] FWidgetTemplateClass::GetToolTipText() was added in 5.8. For a valid class it just
	// forwards UClass::GetToolTipText() (present in 5.7), which is exactly this branch's input.
	Entry.Description = Class->GetToolTipText();
#else
...
#if UE_VERSION_OLDER_THAN(5, 8, 0)
	// [5.7 port] UWidgetBlueprintExtension::GatherWidgetMemberReferences() was added in 5.8 and has
	// no 5.7 equivalent. Phase 5 is an additive enrichment of the reference set; skipping it on 5.7
	// only means extension-contributed references are not gathered (graceful degradation).
	(void)OldClass;
	(void)VariableName;
#else
