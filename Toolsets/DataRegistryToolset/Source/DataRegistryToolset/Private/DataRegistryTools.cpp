...
namespace
{
	// [5.7 port] UDataRegistry::GetDataSources()/GetRuntimeSources() were added as public inline
	// accessors in UE 5.8. In 5.7 the underlying DataSources/RuntimeSources members exist but are
	// protected. A derived helper re-exposes them via `using` (well-defined C++; adds no members).
	class FDataRegistrySourceAccessor : public UDataRegistry
	{
	public:
		using UDataRegistry::DataSources;
		using UDataRegistry::RuntimeSources;
	};

	const TArray<TObjectPtr<UDataRegistrySource>>& GetDataSourcesCompat(const UDataRegistry& Registry)
	{
		return static_cast<const FDataRegistrySourceAccessor&>(Registry).DataSources;
	}

	const TArray<TObjectPtr<UDataRegistrySource>>& GetRuntimeSourcesCompat(const UDataRegistry& Registry)
	{
		return static_cast<const FDataRegistrySourceAccessor&>(Registry).RuntimeSources;
	}
}
...
#if UE_VERSION_OLDER_THAN(5, 8, 0)
	return BuildSourceSummaries(GetDataSourcesCompat(*Registry));
#else
...
#if UE_VERSION_OLDER_THAN(5, 8, 0)
	return BuildSourceSummaries(GetRuntimeSourcesCompat(*Registry));
#else
