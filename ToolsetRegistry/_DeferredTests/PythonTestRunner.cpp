...
namespace UE::ToolsetRegistry
{
		...
		TArray <FString>& OutTestCommands) const
	{
		...
		{
			UE_LOG(LogToolsetRegistry, Error, TEXT("%ls"), *MaybeSearchOptionsCode.GetError());
			return;
		}
		...
		{
			UE_LOG(LogToolsetRegistry, Error, TEXT("%ls"), *MaybeTestIds.GetError());
			return;
		}
		TArray<FString> TestIds = MaybeTestIds.StealValue();
		if (TestIds.Num() == 0)
		{
			UE_LOG(LogToolsetRegistry, Error, TEXT("%ls failed, no tests found for search %ls"),
	...
	bool FPythonAutomationTest::RunTest(const FString& Parameters)
	{
		...
		{
			UE_LOG(LogToolsetRegistry, Error, TEXT("%ls"), *MaybeTestId.GetError());
			return false;
		}
	...
	bool FPythonAutomationTest::PollForTestCompletion(const FString& TestId)
	{
		...
		{
			UE_LOG(LogToolsetRegistry, Error, TEXT("%ls"), *MaybeResult.GetError());
...
bool UPythonTestRunner::CheckInitialized() const
{
	if (!Test.IsValid())
	{
		UE_LOG(LogToolsetRegistry, Error, TEXT("PythonTestRunner must be instanced using create()"));
		return false;
	}
	return true;
}
	...
	float TimeoutInSeconds)
{
	...
	RunTestDelegate =
		[](const FString& TestName) -> void
		{
			...
			{
				UE_LOG(LogToolsetRegistry, Error, TEXT("Unable to run test %ls, engine not available"),
					*TestName);
			}
		};
