...
namespace
{
	...
	FSandboxAccess GetSandboxAccess(bool bLogErrorIfNoManager = false)
	{
		...
		if (!IFileSandboxCoreModule::IsAvailable())
		{
			...
			{
				UE_LOG(LogToolsetRegistry, Error, TEXT("FileSandbox module is not available"));
			}
			return {};
		}
...
namespace UE::ToolsetRegistry
{
	...
	bool FGlobalSandbox::Enter(const FString& Name, const FString& Description)
	{
		...
		if (Access.ActiveInstance)
		{
			...
			{
				UE_LOG(LogToolsetRegistry, Error, TEXT("Failed to leave active sandbox before entering '%ls'"), *Name);
				return false;
			}
		}
		...
		{
			UE_LOG(LogToolsetRegistry, Display, TEXT("Resumed existing sandbox: %ls"), *Name);
			return true;
		}
		...
		{
			UE_LOG(LogToolsetRegistry, Display, TEXT("Created new sandbox: %ls"), *Name);
			return true;
		}

		UE_LOG(LogToolsetRegistry, Error, TEXT("Failed to enter sandbox: %ls"), *Name);
		return false;
	}
	...
	bool FGlobalSandbox::Persist(const TArray<FString>& Files)
	{
		...
		if (!Instance)
		{
			UE_LOG(LogToolsetRegistry, Error, TEXT("No active sandbox to persist"));
			return false;
		}
	...
	bool FGlobalSandbox::Discard()
	{
		...
		if (!Instance)
		{
			UE_LOG(LogToolsetRegistry, Error, TEXT("No active sandbox to discard"));
			return false;
		}
	...
	bool FGlobalSandbox::DiscardFiles(const TArray<FString>& Files)
	{
		if (Files.IsEmpty())
		{
			UE_LOG(LogToolsetRegistry, Error, TEXT("DiscardFiles called with empty file list; ignoring to avoid discarding all sandbox changes"));
			return false;
		}
		...
		if (!Instance)
		{
			UE_LOG(LogToolsetRegistry, Error, TEXT("No active sandbox to discard files from"));
			return false;
		}
		...
		UE_LOG(LogToolsetRegistry, Verbose, TEXT("RevertSandbox: %d package(s) pending hot-reload, %d package(s) pending purge"),
