...
TSharedPtr<FJsonValue> UToolCallAsyncResult::GetValueAsJson() const
{
	...
	if (!Property)
	{
		UE_LOG(LogToolsetRegistry, Error, TEXT("Unable to get Value property from class '%ls'"),
			*Class->GetName());
...
bool UToolCallAsyncResult::SetError(const FString& InError)
{
	if (InError.Len() == 0)
	{
		UE_LOG(LogToolsetRegistry, Error, TEXT("Attempted to complete '%ls' with an empty error!"),
			*GetClass()->GetName());
		return false;
	}
	...
	TFunction<void()>&& SetFinalState)
{
	if (GetIsComplete())
	{
		UE_LOG(LogToolsetRegistry, Error, TEXT("Unable to complete '%ls' more than once"),
			*GetClass()->GetName());
		return false;
	}
	TStrongObjectPtr<UToolCallAsyncResult> Result(this);
