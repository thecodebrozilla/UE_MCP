...
void UToolsetRegistry::RegisterToolsetClass(TSubclassOf<UToolsetDefinition> InToolsetClass)
{
	if (!InToolsetClass)
	{
		UE_LOG(LogToolsetRegistry, Error, TEXT("Cannot register null toolset class"));
		return;
	}
		...
		ToolsetRegistrySubsystem.HasValue())
	{
		...
		{
			UE_LOG(LogToolsetRegistry, Warning, TEXT("Unable to register Toolset %ls: invalid tool list"), *ToolsetName);
			return;
		}

		if (ToolsetRegistrySubsystem.GetValue()->ToolsetRegistry.RegisterToolset(Toolset))
		{
			UE_LOG(LogToolsetRegistry, Display, TEXT("Registering Toolset %ls"), *ToolsetName);
		}
		else
		{
			UE_LOG(LogToolsetRegistry, Warning, TEXT("Unable to register Toolset %ls"), *ToolsetName);
		}
	}
	else
	{
		UE_LOG(LogToolsetRegistry, Error, TEXT("AIToolsetRegistrySubsystem unavailable"));
	}
}
...
void UToolsetRegistry::UnregisterToolsetClass(TSubclassOf<UToolsetDefinition> InToolsetClass)
{
	...
	if (!InToolsetClass)
	{
		UE_LOG(LogToolsetRegistry, Warning, TEXT("Attempting to unregister null toolset class"));
		return;
	}
		...
		ToolsetRegistrySubsystem.HasValue())
	{
		...
		if (Handler)
		{
			UE_LOG(LogToolsetRegistry, Display, TEXT("Unregistering Toolset %ls"), *ToolsetName);
...
FString UToolsetRegistry::GetToolsetJsonSchema(TSubclassOf<UToolsetDefinition> InToolsetClass)
{
	if (!InToolsetClass)
	{
		UE_LOG(LogToolsetRegistry, Warning, TEXT("Attempting to get schema of null toolset class"));
		return FString();
	}
