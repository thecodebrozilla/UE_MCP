...
namespace UE::ModelContextProtocol::Private
{
	...
	bool WriteJsonClientConfiguration(const FString& FilePath, const FClientConfigDescriptor& Descriptor, const FString& ServerUrl)
	{
		...
		if (FFileHelper::LoadFileToString(ExistingContent, *FilePath))
		{
			...
			{
				UE_LOG(LogModelContextProtocol, Warning, TEXT("Existing file contains malformed JSON, overwriting: %ls"), *FilePath);
				RootObject.Reset();
			}
		}
		...
		{
			UE_LOG(LogModelContextProtocol, Warning, TEXT("Failed to serialize JSON for: %ls"), *FilePath);
			return false;
		}
		...
		{
			UE_LOG(LogModelContextProtocol, Warning, TEXT("Failed to create directory: %ls"), *Directory);
			return false;
		}

		if (FFileHelper::SaveStringToFile(OutputString, *FilePath))
		{
			UE_LOG(LogModelContextProtocol, Display, TEXT("MCP client configuration written to: %ls"), *FilePath);
			return true;
		}

		UE_LOG(LogModelContextProtocol, Warning, TEXT("Failed to write MCP client configuration to: %ls"), *FilePath);
		return false;
	}
	...
	bool WriteTomlClientConfiguration(const FString& FilePath, const FString& ServerUrl)
	{
		...
		{
			UE_LOG(LogModelContextProtocol, Error, TEXT("Codex configuration already exists at: %ls. TOML format does not support safe upsert — edit manually to add [mcp_servers.%ls]."), *FilePath, ServerEntryName);
			return false;
		}
...
bool UE::ModelContextProtocol::WriteClientConfiguration(EModelContextProtocolClient Client, uint32 Port, const FString& UrlPath, const FString& BaseDirectory)
{
	...
	{
		UE_LOG(LogModelContextProtocol, Warning, TEXT("Unknown MCP client type"));
		return false;
	}
