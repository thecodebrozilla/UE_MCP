...
uint32 UE::ModelContextProtocol::GetServerPortNumber()
{
	...
	if (FParse::Value(FCommandLine::Get(), TEXT("ModelContextProtocolPort="), Port))
	{
		...
		UE_LOG(LogModelContextProtocol, Warning, TEXT("Invalid command-line port %u. Must be 1-65535. Falling back to settings."), Port);
	}
...
bool UE::ModelContextProtocol::ShouldAutoStartServer()
{
	...
	if (FParse::Param(FCommandLine::Get(), TEXT("StartModelContextProtocolServer")))
	{
		...
		if (!bDeprecationWarned)
		{
			...
			UE_LOG(LogModelContextProtocol, Warning, TEXT("-StartModelContextProtocolServer is deprecated; use -ModelContextProtocolStartServer instead."));
		}
		return true;
	}
...
void UModelContextProtocolSettings::EnforceValidServerUrlPath(FString& InOutPath)
{
	...
	{
		UE_LOG(LogModelContextProtocol, Warning, TEXT("ServerUrlPath '%ls' is not a valid HTTP path; falling back to '%ls'."), *InOutPath, UE::ModelContextProtocol::DefaultServerUrlPath);
