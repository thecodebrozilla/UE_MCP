...
namespace UE::ModelContextProtocol::Private
{
	...
	FAutoConsoleCommand CommandGenerateClientConfig = FAutoConsoleCommand(
		...
		FConsoleCommandWithArgsDelegate::CreateLambda([](const TArray<FString>& Args)
		{
			if (Args.IsEmpty())
			{
				UE_LOG(LogModelContextProtocol, Warning, TEXT("Usage: ModelContextProtocol.GenerateClientConfig <ClaudeCode|Cursor|VSCode|Gemini|Codex|All>"));
				return;
			}
			...
			if (Args[0].Equals(TEXT("All"), ESearchCase::IgnoreCase))
			{
				...
				UE_LOG(LogModelContextProtocol, Display, TEXT("Generated %d MCP client configuration file(s)"), Count);
			}
			else
			{
				...
				if (!Client.IsSet())
				{
					UE_LOG(LogModelContextProtocol, Warning, TEXT("Unknown client \"%ls\". Supported: ClaudeCode, Cursor, VSCode, Gemini, Codex, All"), *Args[0]);
					return;
				}
...
private:
	void RegisterDefaultAnalyticsProvider()
	{
		...
		{
			UE_LOG(LogModelContextProtocol, Log, TEXT("Analytics provider already set; not overriding with FEngineAnalytics proxy default."));
			return;
		}
		...
		UE_LOG(LogModelContextProtocol, Log, TEXT("Registered FEngineAnalytics proxy as the default MCP analytics provider."));
	}
