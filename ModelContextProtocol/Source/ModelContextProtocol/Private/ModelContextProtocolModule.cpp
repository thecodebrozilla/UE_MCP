		...
		FConsoleCommandWithArgsDelegate::CreateLambda([](const TArray<FString>& Args)
		{
			if (IModelContextProtocolModule* Module = IModelContextProtocolModule::Get())
			{
				if (Args.Num() > 0)
				{
					...
					{
						UE_LOG(LogModelContextProtocol, Warning, TEXT("Invalid port %d. Must be 1-65535."), ParsedPort);
					}
				}
				else
				{
...
bool FModelContextProtocolModule::AddTool(const TSharedRef<IModelContextProtocolTool>& Tool)
{
	...
	{
		UE_LOG(LogModelContextProtocol, Warning, TEXT("A ModelContextProtocol tool named \"%ls\" is already registered. All tool names must be globally unique"), *ToolName);
		return false;
	}
	...
	case UE::ModelContextProtocol::EToolNameValidation::Empty:
		UE_LOG(LogModelContextProtocol, Warning, TEXT("Rejecting tool with empty name; MCP spec requires 1-128 characters"));
		return false;
	case UE::ModelContextProtocol::EToolNameValidation::ExceedsMaxLength:
		UE_LOG(LogModelContextProtocol, Warning, TEXT("Tool name \"%ls\" exceeds MCP spec limit of 128 characters (%d)"), *ToolName, ToolName.Len());
		break;
	case UE::ModelContextProtocol::EToolNameValidation::InvalidCharacters:
		UE_LOG(LogModelContextProtocol, Log, TEXT("Tool name \"%ls\" contains characters outside MCP spec (allowed: A-Z, a-z, 0-9, _, -, .)"), *ToolName);
		break;
	default:
		break;
	}
