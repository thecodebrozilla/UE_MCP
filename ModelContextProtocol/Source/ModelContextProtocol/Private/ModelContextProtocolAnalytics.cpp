...
namespace UE::ModelContextProtocol::Analytics
{
	...
	bool IsToolResultSuccess(const TSharedPtr<FJsonObject>& ResultJson)
	{
		...
		{
			UE_LOG(LogModelContextProtocol, Warning, TEXT("Tool result has non-bool 'isError' field; treating as error."));
			return false;
		}
	...
	static void RecordEvent(const FString& EventName, const TArray<FAnalyticsEventAttribute>& Attributes)
	{
		if (!bEnableAnalytics)
		{
			UE_LOG(LogModelContextProtocol, VeryVerbose, TEXT("Analytics disabled (ModelContextProtocol.EnableAnalytics=false); skipping event '%ls'"), *EventName);
			return;
		}
		...
		{
			UE_LOG(LogModelContextProtocol, VeryVerbose, TEXT("No analytics provider set; skipping event '%ls'"), *EventName);
			return;
		}

		UE_LOG(LogModelContextProtocol, VeryVerbose, TEXT("Recording analytics event: %ls"), *EventName);
