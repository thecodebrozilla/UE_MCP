...
int32 UModelContextProtocolToolHashMappingCommandlet::Main(const FString& Params)
{
	...
	if (!Module)
	{
		UE_LOG(LogModelContextProtocolToolHashMapping, Error, TEXT("IModelContextProtocolModule is not available; cannot enumerate tools."));
		return 1;
	}
	...
	{
		UE_LOG(LogModelContextProtocolToolHashMapping, Display, TEXT("Ticking the engine for %.2fs to let -ExecCmds, game feature activations, and toolset registrations complete."), TickEngineSeconds);
	...
	if (Names.IsEmpty())
	{
		UE_LOG(LogModelContextProtocolToolHashMapping, Error, TEXT("Module->GetTools() returned no tools after RefreshTools()."));
		return 1;
	}
	...
	{
		UE_LOG(LogModelContextProtocolToolHashMapping, Error, TEXT("Failed to create output directory: %ls"), *OutputDir);
		return 1;
	}
	...
	{
		UE_LOG(LogModelContextProtocolToolHashMapping, Error, TEXT("Failed to write mapping to: %ls"), *OutputPath);
		return 1;
	}

	UE_LOG(LogModelContextProtocolToolHashMapping, Display, TEXT("Wrote %d tool hashes and %d toolset hashes to %ls"), Tools.Num(), Toolsets.Num(), *OutputPath);

	if (FParse::Param(*Params, TEXT("Print")))
	{
		UE_LOG(LogModelContextProtocolToolHashMapping, Display, TEXT("%ls"), *Json);
	}
