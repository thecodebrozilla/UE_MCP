...
bool SaveManifest(const FFileSandboxCore_ManifestData& InData, const FString& InRootDirectory)
{
	...
	UE_CLOG(!bSuccess, LogFileSandbox, Error, TEXT("Failed to write sandbox manifest file %ls"), *InRootDirectory);
	return bSuccess;
}
...
TOptional<FFileSandboxCore_ManifestData> LoadManifest(const FString& InRootDirectory, bool bLogErrorIfNotExist)
{
	...
	if (FFileHelper::LoadFileToString(JSONPayload, *AbsFileName))
	{
		...
		UE_CLOG(!Result.IsSet(), LogFileSandbox, Error, TEXT("Failed to parse sandbox manifest file %ls"), *InRootDirectory);
		return Result;
	}
	
	UE_CLOG(bLogErrorIfNotExist, LogFileSandbox, Error, TEXT("Failed to load sandbox manifest file %ls"), *InRootDirectory);
	return {};
}
