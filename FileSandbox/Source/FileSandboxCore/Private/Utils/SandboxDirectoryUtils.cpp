	...
	)
{
	...
	UE_CLOG(!bSuccess, LogFileSandbox, Error, TEXT("Failed to write sandbox metadata file %ls"), *AbsFileName);
	return bSuccess;
}
...
TOptional<FFileSandboxCore_SandboxMetaData> LoadMetaData(const FString& InSandboxDirectory)
{
	...
	if (FFileHelper::LoadFileToString(JSONPayload, *AbsFileName))
	{
		...
		UE_CLOG(!Result.IsSet(), LogFileSandbox, Error, TEXT("Failed to parse sandbox metadata file %ls"), *AbsFileName);
		return Result;
	}
	return {};
}
