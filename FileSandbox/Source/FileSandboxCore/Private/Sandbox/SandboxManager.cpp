...
FLoadSandboxResult FSandboxManager::LoadSandbox(const FLoadSandboxByDirectoryArgs& InLoadArgs)
{
	...
	{
		UE_LOG(LogFileSandbox, Error, TEXT("Directory %ls does not contain any sandbox"), *AbsoluteRoot);
	...
	UE_CLOG(LastDirectoryName.IsEmpty(), LogFileSandbox, Warning, TEXT("Sandbox placed %ls will not have any name."), *AbsoluteRoot);
	...
	if (SandboxVersionInfo.IsInitialized())
	{
		...
		{
			UE_LOG(LogFileSandbox, Error, TEXT("Cannot load sandbox at %ls: Sandbox was created with version %ls but current version is %ls"),
				*AbsoluteRoot,
