...
#if !UE_VERSION_OLDER_THAN(5, 8, 0) // [5.7 port] ISourceControlProvider::UsesSoftRevertOnDelete is 5.8+
...
#if !UE_VERSION_OLDER_THAN(5, 8, 0) // [5.7 port] HasChangesToSync/HasChangesToCheckIn are 5.8+
...
#endif
#if UE_VERSION_OLDER_THAN(5, 8, 0) // [5.7 port] removed in 5.8 (superseded by HasChangesToSync/HasChangesToCheckIn)
	virtual TOptional<bool> IsAtLatestRevision() const override
	{
		WRAP_RET({}, IsAtLatestRevision)
	}
	virtual TOptional<int32> GetNumLocalChanges() const override
	{
		WRAP_RET({}, GetNumLocalChanges)
	}
#endif
